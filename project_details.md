...
>
> This thesis document was originally written during the planning phase. As the
> implementation progressed across Sprints 1–11, **38 ADRs** logged decisions
> and deviations from the original plan. This preface lists the most material
> deviations between the thesis text below and the actual shipped MVP, so
> supervisors and external reviewers can read the doc with the right context.
>
> Full ADR text lives in [`docs/decisions.md`](docs/decisions.md). Sprint-by-sprint
> progress is in [`docs/progress.md`](docs/progress.md).
>
> ### Sprint-by-sprint deviation summary
>
> | Sprint | ADRs introduced | Headline deviation from thesis text |
> |---|---|---|
> | Sprint 1 (Foundations) | ADR-001 .. ADR-014 | .NET 10 chosen (was .NET 8 in thesis); Identity entities live in Infrastructure not Domain (ADR-010); GitHub OAuth + rate-limit deferred from S1 → S2 (ADR-011); Redis-backed rate-limit deferred to post-MVP (ADR-012). |
> | Sprint 2 (Assessment) | — | Question bank seeded at 60 (≥60 target met). 30-day reattempt policy enforced. |
> | Sprint 3 (Learning Path) | ADR-015 .. ADR-019 | Hangfire-driven path generation (ADR-016); weakest-first ordering algorithm (ADR-017); Redis-backed task-list cache with version-counter invalidation (ADR-018); safe-subset markdown renderer in FE (ADR-019, no `dangerouslySetInnerHTML`). |
> | Sprint 4 (Submission Pipeline) | ADR-020, ADR-021 | Path-update rules formalized (ADR-020); submission-analysis scheduler abstraction (ADR-021). |
> | Sprint 5 (AI Integration + Static) | ADR-022 .. ADR-025 | `/api/review` renamed to `/api/ai-review` (ADR-022); per-tool response shape (ADR-024); graceful AI-outage degradation with auto-retry (ADR-025). |
> | Sprint 6 (AI Review + Feedback) | ADR-026, ADR-027 | PathTask auto-completion threshold = AI overall ≥70 (ADR-026); prompt versioning + 5 PRD F6 score names — `correctness/readability/security/performance/design` — supersede the original 5 thesis category names (ADR-027). |
> | Sprint 7 (Learning CV + Admin) | ADR-028 | Code-quality skill axis added alongside assessment axis (ADR-028, supersedes ADR-026 stance on submission AI scores into SkillScore). Public CV slug derived from username with reserved-slug list. |
> | Sprint 8 (Stretch + MVP Hardening) | ADR-029 | XP level curve: `level = floor(sqrt(xp/50))+1`; 5 hand-designed badges (ADR-029). RFC 7807 ProblemDetails everywhere. **M2 milestone reached 2026-04-27.** |
> | Sprint 9 (Project Audit / F11) | ADR-031 .. ADR-035 | F11 inserted post-M2 (ADR-031, ADR-032); 90-day blob retention via Hangfire recurring job (ADR-033); audit-tone prompt distinct from per-task review prompt (ADR-034); combined static+audit endpoint response shape (ADR-035). |
> | Sprint 10 (RAG Mentor Chat / F12) | ADR-036 | F12 added as differentiation feature: Qdrant + `text-embedding-3-small` + RAG retrieval + SSE streaming with raw-fallback mode when no chunks retrieved (ADR-036). New entities `MentorChatSessions` + `MentorChatMessages` with polymorphic scoping over Submissions/ProjectAudits. |
> | Sprint 11 (Multi-Agent / F13 + defense prep) | ADR-037, ADR-038 | F13 added — three specialist agents (security / performance / architecture) running in parallel via `asyncio.gather`, merged into the existing review response shape with optional `meta` block (ADR-037). Endpoint pair: `/api/ai-review-multi` + `/api/analyze-zip-multi`, opt-in via `AI_REVIEW_MODE` env var (default `single` for cost containment). **Azure deployment deferred to a Post-Defense slot — defense runs locally on the owner's laptop (ADR-038).** M3 redefined accordingly. |
>
> ### Specific section-level deviations to flag while reading
>
> - **Chapter 4 (System Design) — Tech Stack:** the implementation runs on .NET 10 + Hangfire (background jobs) + Redis (cache + rate limit) + SQL Server 2022 (primary store) + Azurite (Blob, dev) + Seq (structured logs) + Qdrant (vector store, F12) + FastAPI (AI service). Some thesis text predates F12 + F13; the architecture document at [`docs/architecture.md`](docs/architecture.md) is authoritative.
> - **Score categories:** thesis pre-Sprint 6 used `functionality / readability / bestPractices / security / performance`. The shipped MVP uses the 5 PRD F6 names: `correctness / readability / security / performance / design` (ADR-027). Single-prompt and multi-agent paths both emit the same 5 names.
> - **AI review architecture:** thesis describes a single-prompt review (F6, Sprint 6 deliverable). Sprint 11 adds **F13 — Multi-Agent Code Review** (ADR-037) as a parallel endpoint with a thesis A/B comparison harness under [`tools/multi-agent-eval/`](tools/multi-agent-eval/). The evaluation report scaffold lives at [`docs/demos/multi-agent-evaluation.md`](docs/demos/multi-agent-evaluation.md).
> - **F11 (Project Audit):** added post-M2 (ADR-031), so any "10 MVP features" framing in the thesis is now **13 MVP features**: F1–F10 + F11 (audit) + F12 (mentor chat) + F13 (multi-agent review).
> - **Deployment chapter (later in this doc):** the chapter describes the Azure deployment plan. Per **ADR-038**, this work is **deferred to a Post-Defense slot** — defense itself runs locally on the owner's laptop. The deployment plan is preserved as the post-graduation continuation backlog (see "Future Work" appendix at the end of this doc).
>
> ### How to read the rest of this document
>
> Read the original thesis text below for the design rationale and academic
> framing. When the implementation diverges, this preface tells you so + points
> at the ADR. The Future Work appendix at the end consolidates everything
> consciously deferred (Azure deployment, multi-provider AI, full badge
> catalogue, k8s, etc.) so reviewers see the boundary between what the MVP
> ships and what is consciously next.
>
> **A complete section-by-section reconciliation pass — rewriting each
> chapter to match implementation reality verbatim — is the carryover work
> with supervisors before defense rehearsal (S11-T7 owner-led carryover).
> The structural skeleton (this preface + Future Work appendix) is in place;
> deeper section edits land iteratively as supervisors flag specific text.**
...
# **Future Work** *(appended Sprint 11 — captures consciously-deferred items per ADR-038 and the post-MVP roadmap in PRD §5.3)*

This appendix consolidates everything the MVP intentionally defers, so reviewers can see the boundary between what ships at defense and what is next. Each item below has a clear gate and an owner.

## Immediate Post-Defense — Azure Deployment Slot *(per ADR-038)*

Originally Sprint 10 in the implementation plan, **deferred to post-defense** because (a) defense runs locally on the owner's laptop, (b) Sprints 10–11 capacity went into F12 (Mentor Chat) and F13 (Multi-Agent Review) which produce stronger differentiation than provisioning, and (c) Azure for Students credit (~$100) is preserved for actual hosting after graduation. Tasks (renumbered as PD-T1..PD-T11):

- **PD-T1** Provision Azure resources via Bicep or portal: resource group, App Service B1, Azure SQL Basic, Azure Cache for Redis Basic C0, Azure Blob, Key Vault, **Qdrant on Azure Container Instances or Railway** (ADR-036).
- **PD-T2** Dockerize backend; push to Azure Container Registry.
- **PD-T3** App Service: env vars from Key Vault (incl. `QDRANT_URL`, `OPENAI_API_KEY`, `AI_REVIEW_MODE` defaulted to `single`), DB-migration-on-startup guard, Hangfire credentials, Redis connection.
- **PD-T4** Deploy frontend to Vercel; CORS on backend.
- **PD-T5** Deploy AI service to Railway or Azure Container Instances; Qdrant reachable from AI service over private networking.
- **PD-T6** Internal service auth (shared secret header backend ↔ AI service).
- **PD-T7** k6 load test against Azure: 100 concurrent users on B1 tier; document p95 latency; identify whether B1 suffices.
- **PD-T8** Application Insights: 5 dashboards (request rate, p95 latency, error rate, submission pipeline duration, OpenAI token consumption — already split into 3 series per S11-T5: `ai-review`, `ai-review-multi`, `project-audit`).
- **PD-T9** Fix Azure-tier bottlenecks (likely candidates: SQL indexes, cache keys, Qdrant query tuning, Hangfire pool sizing).
- **PD-T10** Custom domain + TLS (optional).
- **PD-T11** Smoke-test every MVP feature on Azure (F1–F13).

**Exit criteria when this slot is later activated:** all 13 MVP features demoable on Azure URLs; load-test report committed; monitoring dashboards live; team has access to a stable URL for post-graduation continuation.

## Post-MVP Roadmap *(PRD §5.3 — named in thesis as future work)*

| Area | Deferred item |
|---|---|
| Engagement | Streaks, peer benchmarking, full badge catalog (the MVP ships 5 starter badges per ADR-029) |
...
| Infra | Azure Service Bus (replaces Hangfire's polling for higher throughput), split Worker process, full CI/CD pipeline, SonarQube integration, k8s |
...
- **F12 Mentor Chat (ADR-036) extensions:** voice input/TTS output, mentor chat scoped to a learning task (currently scoped only to submissions and project audits), cross-session "ask the mentor about my whole history," embedding-based similar-submission lookup.
- **F13 Multi-Agent Review (ADR-037) extensions:** five-agent variant (separate agents per category instead of three), sequential rather than parallel orchestration with the architecture agent summarizing first, automatic mode-switching based on submission complexity, agent-specific prompt versioning surfaced in the UI, per-agent score breakdowns surfaced to learners.
...
- Larger N (50–100 submissions) for statistical-significance claims beyond the thesis-window comparison.
- Domain-specific analysis: which language / project type / category benefits most from specialist decomposition.
- Inter-rater reliability work with a third blind supervisor.
- Cost vs quality Pareto frontier — at what budget does multi-agent stop paying off?

These are explicitly **not** in defense scope; they are research-paper extensions for the team if the project is published post-graduation.
